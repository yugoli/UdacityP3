...
ggplot(data=l, aes(x=CreditScoreRangeLower)) +geom_histogram(binwidth=20) +
  xlim(400, 880)
...
I subsetted only delinquent loans (loans that were labelled Past Due). It turns
...
ggplot(data=l, aes(x=CurrentCreditLines, y=TotalMonthlyDebt)) +
  geom_jitter(alpha=0.1, size= 1.5, position = 'jitter') +ylim(0, 7000)+
  xlim(0, 30) 
```
...
ggplot(data=l, aes(x=LoanYear, y=LoanOriginalAmount)) + 
...
```{r,loan amt catyr, echo=FALSE, fig.width = 8, fig.height = 10, warning=FALSE}
byyear <- l %>%
...
ggplot(data=byyear, aes(x=LoanYear, y=DollarTotal/1000000, 
                        fill=ListingCategory..numeric.)) +
  geom_bar(stat='identity') + 
  scale_fill_discrete(name="Loan Category", labels=cats) +
  ggtitle("Loan Amount by Category 2005-2014") + 
  ylab("Dollar Total $ '000,000 ") + xlab("Year")
```

### Description Three
I bucketed loan categories by year and you can clearly see that it looks like 
pre-2008 they did not capture the loan category and debt consolidation
dominated the cateogory every year subsequently. 2013 was the best full year for 
Prosper, originating over $350M loans. Within the first 3 months of 2014,
the platform originated nearly $150M loans, on pace for another record year. 
