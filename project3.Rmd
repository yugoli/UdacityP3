...
It is also interesting in the loans by year plot that the loans dropped off in 2009. I would guess that is a supply side shortage due to the Financial Crisis. What do you think? (Hint: I'm wrong.)
...
```{r,  loan normalized by credit score by income range , echo=FALSE, message=FALSE , warning=FALSE}


ggplot(data=l, aes(y=LoanOriginalAmount*CreditScoreRangeLower/850, x=IncomeRange, fill=IncomeRange )) +geom_boxplot()+scale_x_discrete(labels="")
```
...
```{r,  debt coverage , echo=FALSE, message=FALSE , warning=FALSE}
ggplot(data=subset(l, l$IncomeRange != '$0'), aes(x=LoanYear, y= MonthlyLoanPayment/StatedMonthlyIncome, fill=IncomeRange)) +geom_boxplot() +ylim(0, 0.1) +facet_wrap(~IncomeRange)
```
...
```{r,  di ratio , echo=FALSE, message=FALSE , warning=FALSE}

ggplot(data=subset(l, l$IncomeRange != '$0'), aes(x=LoanYear, y= DebtToIncomeRatio, fill=IncomeRange)) +geom_boxplot() +ylim(0, .5) +facet_wrap(~IncomeRange)

```

Comparing DI ratio, most of the borrowers seem to have DI ratio close to 20-30% with a uptick in the most recent years. The $100k + income range have noticeably lower DI ratio at around 15-20%.   
...
ggplot(data=subset(byyear, byyear$IncomeRange != '$0'), aes(x=LoanYear, y=DollarTotal, fill=IncomeRange)) +geom_bar(stat='identity') + scale_fill_discrete(name="Income Range")+ggtitle("Loan Amount by Year and Income Range") +facet_wrap(~IncomeRange)
...
### Plot One
```{r, Borrowers segmentation Final, echo=FALSE, message=FALSE , warning=FALSE}
ggplot(data=subset(l, l$CreditScoreRangeLower > 660), aes(x=BorrowerRate, y=LoanOriginalAmount, color=CreditScoreRangeLower)) +geom_point(alpha=0.5, position='jitter') + scale_colour_gradient(low="blue", high="red") +ggtitle("Loan Amount by Credit Score and Interest Rate") +facet_wrap(~LoanYear)
```
...
### Plot Two
```{r,  Debt Category Final, echo=FALSE, message=FALSE , warning=FALSE}

ggplot(data=subset(l, l$DebtToIncomeRatio < .4), aes(x=BorrowerRate, y=LoanOriginalAmount, color=DebtToIncomeRatio)) +geom_point(alpha=0.5, position='jitter') +facet_wrap(~LoanYear) + scale_colour_gradient(low="green", high="red") +ggtitle("Loan Amount by DI Ratio and Interest Rate")
...
### Description Two
Looking at DI (debt to income) ratio, it illustrates a picture of increased loan sizing, especially at lower borrowing rates (2013-2014) as the platform matured and defaults rate were decreasing (graphed before).



### Plot Three
```{r,  credit score by income Final, echo=FALSE, message=FALSE , warning=FALSE}
delin <- l[grep("Past Due", l$LoanStatus),] # only delinquent

ggplot(data=subset(delin, delin$IncomeRange != '$0'), aes(x=LoanYear, y=CreditScoreRangeLower)) +geom_point(aes(color=LoanOriginalAmount),size=2.5,alpha=0.7, position='jitter') +facet_wrap(~IncomeRange) + scale_colour_gradient(low="blue", high="red") +ggtitle("Delinquent loan exposure summary")

```


### Description Three
I subsetted only deliquent loans (total count: 2067) to look at their at risk loans. It appears the majority of their delinquencies are clustered in 2012-2013 to with borrowers income range $25k-50k, $50-75k with credit scores 650-750. 
