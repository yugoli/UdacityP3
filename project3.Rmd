...
ggplot(data=l, aes(x=StatedMonthlyIncome, y=TotalMonthlyDebt)) +geom_point(alpha=0.05, size=1.5, position='jitter') + xlim(0, 15000) + ylim(0, 7000) + facet_wrap(~IsBorrowerHomeowner)
```
...
ggplot(data=subset(l, l$CreditScoreRangeLower > 660), aes(x=BorrowerRate, y=LoanOriginalAmount, color=CreditScoreRangeLower)) +geom_point(alpha=0.1, position='jitter') + scale_colour_gradient(low="blue", high="red") +ggtitle("Loan Amount by Credit Score and Interest Rate") +facet_wrap(~LoanYear)
```
...
ggplot(data=subset(l, l$DebtToIncomeRatio < .4 ), aes(x=BorrowerRate, y=LoanOriginalAmount, color=DebtToIncomeRatio)) +geom_point(alpha=0.1, position='jitter') +facet_wrap(~LoanYear) + scale_colour_gradient(low="green", high="red") +ggtitle("Loan Amount by DI Ratio and Interest Rate") +xlim( 0, 0.4)
